...
Work only inside `apps/parents` unless the task explicitly requires shared package changes.
...
```bash
pnpm --filter @burro/parents typecheck
pnpm --filter @burro/parents build
```
