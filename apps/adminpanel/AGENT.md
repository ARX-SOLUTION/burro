...
Work only inside `apps/adminpanel` unless the task explicitly requires shared package changes.
...
```bash
pnpm --filter @burro/adminpanel typecheck
pnpm --filter @burro/adminpanel build
```
