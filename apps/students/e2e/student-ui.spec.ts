...
async function mockApi(page: Page, overrides: { emptyModules?: boolean; fail?: string[]; slow?: string[] } = {}) {
  await page.route("**/*", async (route) => {
    ...
    if (overrides.fail?.includes(url.pathname)) return fulfill(route, { error: { code: "SERVER_ERROR", message: "Server failed" } }, 500);
    if (url.pathname === "/student/path") return fulfill(route, { modules: overrides.emptyModules ? [] : modules });
    if (url.pathname === "/student/profile") return fulfill(route, profile);
    if (url.pathname === "/student/stats/summary") return fulfill(route, stats);
    if (url.pathname === "/student/dashboard") return fulfill(route, dashboard);
    if (url.pathname === "/leaderboards/global") return fulfill(route, leaderboard);
    if (url.pathname === "/student/attempts/start") return fulfill(route, attempt);
    if (url.pathname === "/student/attempts/attempt-1/answer") return fulfill(route, answer);
    if (url.pathname === "/auth/logout") return fulfill(route, { ok: true });
...
test.describe("Burro student UI QA audit", () => {
  ...
  test("20 core web vitals smoke: no obvious CLS and primary interaction under INP budget", async ({ page }) => {
    ...
    expect(interactionMs).toBeLessThanOrEqual(2000);
  });
});
