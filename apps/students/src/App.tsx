...
type AnswerVisualState = "selected" | "correct" | "wrong" | undefined;
...
export default function App() {
  ...
  const answerState = (option: ExerciseOption): AnswerVisualState => {
    if (!checkResult) {
  ...
  return <StudentShell>
    ...
    {tab === "exercise" && <><ProgressHeader title="Savol 4" progress="60%"/><div className="top-row"><HeartCounter hearts={checkResult?.heartsRemaining ?? 3}/><span className="pill">Practice</span></div><GlassCard><h2>Qaysi harf “Ba” tovushini beradi?</h2><AudioButton/>{exerciseOptions.map((option) => <AnswerOption key={option.id} label={option.label} state={answerState(option)} onClick={() => selectAnswer(option)}/>)}<GradientButton disabled={!selected || isChecking || Boolean(checkResult)} onClick={handleCheckAnswer}>{isChecking ? "Tekshirilmoqda..." : "Tekshirish"}</GradientButton>{checkError && <FeedbackPanel type="wrong" text={checkError}/>} {checkResult && <FeedbackPanel type={checkResult.isCorrect ? "correct" : "wrong"} text={checkResult.isCorrect ? `${checkResult.feedback.message}. +${checkResult.xpDelta} XP` : checkResult.feedback.message}/>} {checkResult && <GradientButton onClick={resetExercise}>Davom etish</GradientButton>}</GlassCard><GlassCard><h3>Talaffuz mashqi</h3><MicrophoneButton/></GlassCard></>}
